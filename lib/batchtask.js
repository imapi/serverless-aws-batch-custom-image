...
function updateContainerProperties(newFunction, functionObject) {
  ...
  if (! newFunction.Properties.ContainerProperties.hasOwnProperty('Command')) {
    ...
    const module_func = functionObject.handler.split('.');
    const func = module_func[module_func.length - 1];
    const module = module_func.slice(0, -1).join('.');
    newFunction.Properties.ContainerProperties.Command = [
        "python",
        "-c",
        `import sys; from ${module} import ${func}; ${func}(sys.argv[1]);`,
        "Ref::event"
    ]
  }
