...
function getDockerLoginToECRCommand() {
    const result = awsCommand(['ecr', 'get-login', '--profile',
        this.provider.getProfile(), '--region', this.options.region, '--no-include-email']);
...
function deleteAllDockerImagesInECR() {
    try {
        const result = awsCommand(['ecr', 'list-images',
            '--profile',
            this.provider.getProfile(), '--repository-name', this.provider.naming.getECRRepositoryName()]);
