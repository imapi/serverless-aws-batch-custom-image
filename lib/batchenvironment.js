...
function generateBatchServiceRole() {
    const batchServiceRoleName = `BatchServiceRole-${this.provider.serverless.service.service}-${this.provider.getRegion()}-${this.provider.getStage()}`.substring(0, 64);
...
function generateBatchInstanceRole() {
    const batchInstanceRoleName = `BatchInstanceRole-${this.provider.serverless.service.service}-${this.provider.getRegion()}-${this.provider.getStage()}`.substring(0, 64);
...
function generateBatchSpotFleetRole() {
    const batchSpotRoleName = `BatchSpotFleetRole-${this.provider.serverless.service.service}-${this.provider.getRegion()}-${this.provider.getStage()}`.substring(0, 64);
...
function generateBatchJobExecutionRole() {
    const batchJobExecutionRoleName = `BatchJobRole-${this.provider.serverless.service.service}-${this.provider.getRegion()}-${this.provider.getStage()}`.substring(0, 64);
...
function generateLambdaScheduleExecutionRole() {
    const lambdaScheduleExecutionRoleName = `BatchScheduleRole-${this.provider.serverless.service.service}-${this.provider.getRegion()}-${this.provider.getStage()}`.substring(0, 64);
   ...
   `;
    ...
    let statements = [];
    if (this.serverless.service.provider.hasOwnProperty('iamRoleStatements')) {
        statements = this.serverless.service.provider.iamRoleStatements;
    }
    statements = statements.concat(JSON.parse(policy));
    const lambdaScheduleExecutionRole = JSON.parse(lambdaScheduleExecutionRoleTemplate);

    _.merge(
        lambdaScheduleExecutionRole.Properties,
        {
            "Policies": [
                {
    ...
    // Setup the JobQueue to push tasks to
    return {
        [this.provider.naming.getLambdaScheduleExecutionRoleLogicalId()]: lambdaScheduleExecutionRole
    };
}
